<?php if($orders->count() > 0): ?>
<div class="table-responsive">
    <table class="table table-hover">
        <thead class="table-success">
            <tr>
                <th>Order ID</th>
                <th>Customer</th>
                <th>Driver</th>
                <th>Return Reason</th>
                <th>Return Date</th>
                <th>Amount</th>
                <th>Status</th>
                <th>Actions</th>
            </tr>
        </thead>
        <tbody>
            <?php $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr>
                <td>
                    <a href="<?php echo e(route('admin.returns.show', $order->id)); ?>" class="text-decoration-none">
                        <strong>#<?php echo e($order->id); ?></strong>
                    </a>
                </td>
                <td>
                    <div>
                        <strong><?php echo e($order->user->name); ?></strong><br>
                        <small class="text-muted"><?php echo e($order->user->email); ?></small>
                    </div>
                </td>
                <td>
                    <?php if($order->returnedByDriver): ?>
                        <strong><?php echo e($order->returnedByDriver->name); ?></strong>
                    <?php else: ?>
                        <span class="text-muted">Unknown</span>
                    <?php endif; ?>
                </td>
                <td>
                    <span class="badge" style="background-color: #ffc107; color: #000; font-weight: 600;"><?php echo e($order->return_reason); ?></span>
                    <?php if($order->return_notes): ?>
                        <br><small class="text-muted"><?php echo e(Str::limit($order->return_notes, 30)); ?></small>
                    <?php endif; ?>
                </td>
                <td>
                    <small><?php echo e($order->returned_at ? $order->returned_at->format('M d, Y H:i') : 'N/A'); ?></small>
                </td>
                <td>
                    <strong>₱<?php echo e(number_format($order->total_price, 2)); ?></strong>
                </td>
                <td>
                    <?php switch($order->return_status):
                        case ('pending'): ?>
                            <span class="badge bg-info">Pending Review</span>
                            <?php break; ?>
                        <?php case ('approved'): ?>
                            <span class="badge bg-success">Approved</span>
                            <?php break; ?>
                        <?php case ('rejected'): ?>
                            <span class="badge bg-danger">Rejected</span>
                            <?php break; ?>
                        <?php case ('resolved'): ?>
                            <span class="badge bg-secondary">Resolved</span>
                            <?php break; ?>
                        <?php default: ?>
                            <span class="badge bg-secondary"><?php echo e(ucfirst($order->return_status)); ?></span>
                    <?php endswitch; ?>
                </td>
                <td>
                    <div class="btn-group" role="group">
                        <a href="<?php echo e(route('admin.returns.show', $order->id)); ?>" 
                           class="btn btn-outline-primary btn-sm" title="View Details">
                            <i class="fas fa-eye"></i>
                        </a>
                        
                        <?php if($order->return_status === 'pending'): ?>
                            <button class="btn btn-outline-success btn-sm" 
                                    onclick="showReturnAction(<?php echo e($order->id); ?>, '<?php echo e($order->return_status); ?>')" 
                                    title="Take Action">
                                <i class="fas fa-cog"></i>
                            </button>
                        <?php endif; ?>
                        
                        <?php if($order->return_status === 'approved' && !$order->refund_processed_at): ?>
                            <button class="btn btn-outline-warning btn-sm" 
                                    onclick="showRefundModal(<?php echo e($order->id); ?>, <?php echo e($order->total_price); ?>)" 
                                    title="Process Refund">
                                <i class="fas fa-money-bill-wave"></i>
                            </button>
                        <?php endif; ?>
                        
                        <?php if($order->refund_processed_at): ?>
                            <span class="badge bg-success" title="Refund Processed">
                                <i class="fas fa-check"></i> Refunded
                            </span>
                        <?php endif; ?>
                    </div>
                </td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </tbody>
    </table>
</div>

<!-- Pagination -->
<?php if(method_exists($orders, 'hasPages') && $orders->hasPages()): ?>
<div class="d-flex justify-content-center mt-3">
    <?php echo e($orders->links()); ?>

</div>
<?php endif; ?>

<?php else: ?>
<div class="text-center py-5">
    <i class="fas fa-inbox display-1 text-muted"></i>
    <h5 class="mt-3 text-muted">No returned orders found</h5>
    <p class="text-muted">
        <?php if($filter === 'pending'): ?>
            No orders are pending review at the moment.
        <?php elseif($filter === 'approved'): ?>
            No approved returns found.
        <?php else: ?>
            No returned orders found.
        <?php endif; ?>
    </p>
</div>
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\JJ_Flowershop_Capstone\resources\views/admin/returns/partials/returns_table.blade.php ENDPATH**/ ?>