                                ...
                                @if($hasAlerts)
                                    @if(isset($restockProducts) && count($restockProducts))
                                        @foreach($restockProducts as $product)
                                            <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                                <span>{{ $product->name }}</span>
                                                <span class="badge bg-danger">{{ $product->stock }} left (Min: {{ $product->reorder_min }})</span>
                                            </div>
                                        @endforeach
                                    @endif
                                    @if(isset($overstockProducts) && count($overstockProducts))
                                        @foreach($overstockProducts as $product)
                                            <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                                                <span>{{ $product->name }}</span>
                                                <span class="badge bg-warning text-dark">Exceeds by {{ $product->stock - $product->reorder_max }} (Max: {{ $product->reorder_max }})</span>
                                            </div>
                                        @endforeach
                                    @endif
                                @else
