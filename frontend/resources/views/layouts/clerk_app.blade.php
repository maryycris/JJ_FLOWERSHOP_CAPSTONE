        ...
        <nav class="clerk-navbar-bg">
            <div class="clerk-navbar-content">
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <a href="{{ route('clerk.notifications.index') }}" class="text-white text-decoration-none" title="Notifications">
                        <span class="nav-icon-wrapper" style="font-size: 1.1rem;">
                            <i class="bi bi-bell"></i>
                            @if(($clerkNavCounts['unread_notifications'] ?? 0) > 0)
                                <span class="nav-badge">{{ ($clerkNavCounts['unread_notifications'] > 99) ? '99+' : $clerkNavCounts['unread_notifications'] }}</span>
                            @endif
                        </span>
                    </a>
                    ...
                    <a href="#" class="text-white text-decoration-none" title="Chat">
                        <span class="nav-icon-wrapper" style="font-size: 1.1rem;">
                            ...
                            @if(($clerkNavCounts['unread_messages'] ?? 0) > 0)
                                <span class="nav-badge">{{ ($clerkNavCounts['unread_messages'] > 99) ? '99+' : $clerkNavCounts['unread_messages'] }}</span>
                            @endif
                        </span>
                    </a>
            ...
            <div class="clerk-navbar-links">
                ...
                <a href="{{ route('clerk.inventory.manage') }}" class="clerk-navbar-link @if(request()->routeIs('clerk.inventory.*')) active @endif">
                    <i class="bi bi-box"></i>
                    <span>Inventory</span>
                    @if(($clerkNavCounts['pending_inventory_requests'] ?? 0) > 0)
                        <span class="nav-badge">{{ ($clerkNavCounts['pending_inventory_requests'] > 99) ? '99+' : $clerkNavCounts['pending_inventory_requests'] }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('clerk.orders.index') }}" class="clerk-navbar-link @if(request()->routeIs('clerk.orders.*')) active @endif">
                    ...
                    @if(($clerkNavCounts['pending_orders'] ?? 0) > 0)
                        <span class="nav-badge">{{ ($clerkNavCounts['pending_orders'] > 99) ? '99+' : $clerkNavCounts['pending_orders'] }}</span>
                    @endif
                </a>
