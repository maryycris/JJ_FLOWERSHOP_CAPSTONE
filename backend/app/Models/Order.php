...
class Order extends Model
{
    ...
    /**
     * Get the delivery receipts for this order.
     */
    public function deliveryReceipts()
    {
        return $this->hasMany(\App\Models\DeliveryReceipt::class);
    }
    ...
    /**
     * Get SO number format (SO-00453)
     */
    public function getSoNumberFormatAttribute(): string
    {
        // If order has a sales order with so_number, use that
        if ($this->salesOrder && $this->salesOrder->so_number) {
            return $this->salesOrder->so_number;
        }
        // Otherwise, format as SO-00453
        return 'SO-' . str_pad($this->id, 5, '0', STR_PAD_LEFT);
    }
