...
class ReportController extends Controller
{
    ...
    public function generateDetailedSales(Request $request)
    {
        ...
        $formattedResults = $allResults->map(function($item) {
            // Format SO number - ensure it has "SO-" prefix
            $soNumber = $item->so_number;
            if (empty($soNumber) && isset($item->order_id)) {
                // If no so_number, format order_id as SO-00453
                $soNumber = 'SO-' . str_pad($item->order_id, 5, '0', STR_PAD_LEFT);
            } else if (empty($soNumber)) {
                $soNumber = 'N/A';
            } else {
                // If so_number exists but doesn't start with "SO-", add it
                if (strpos($soNumber, 'SO-') !== 0) {
                    // If it's just numbers, add SO- prefix
                    if (preg_match('/^\d+$/', $soNumber)) {
                        $soNumber = 'SO-' . str_pad($soNumber, 5, '0', STR_PAD_LEFT);
                    } else {
                        // If it has other format, just add SO- prefix
