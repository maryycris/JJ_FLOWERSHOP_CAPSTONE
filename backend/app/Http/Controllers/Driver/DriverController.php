...
class DriverController extends Controller
{
    public function dashboard()
    {
        ...
        $pendingAcceptance = \App\Models\Order::where('assigned_driver_id', $driver->id)
            ...
            ->with(['user.addresses', 'products', 'delivery'])
            ->latest()
            ->get();
    ...
    public function showOrder($orderId)
    {
        ...
        $order->load(['user.addresses', 'delivery', 'products', 'statusHistories']);
            ...
            if ($request->hasFile('proof_of_delivery')) {
                ...
                try {
                    \App\Models\DeliveryReceipt::create([
                        'order_id' => $order->id,
                        'image_path' => $path,
                        'receiver_name' => $request->input('receiver_name') ?? $order->delivery->recipient_name ?? null,
                        'notes' => $request->input('delivery_notes') ?? null,
                        'captured_by' => Auth::id(),
                        'received_at' => now(),
                    ]);
                    \Log::info('Delivery receipt created successfully', ['order_id' => $orderId]);
                } catch (\Exception $e) {
                    \Log::warning('Failed to create delivery receipt', [
                        'order_id' => $orderId,
                        'error' => $e->getMessage()
                    ]);
                    // Don't fail the order completion if receipt creation fails
                }
            }
