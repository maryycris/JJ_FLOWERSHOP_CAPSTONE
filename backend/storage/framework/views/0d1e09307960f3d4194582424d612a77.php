        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (row) {
                    ...
                    // Visual cues for min/max breaches on Qty On Hand cell (index 7)
                    const minVal = parseInt(formData.get('reorder_min') || 0);
                    const maxVal = parseInt(formData.get('reorder_max') || 0);
                    if (cells[7]) {
                        const span = document.createElement('span');
                        span.textContent = stock.toString();
                        span.title = '';
                        span.className = '';
                        if (minVal > 0 && stock < minVal) {
                            span.className = 'text-danger fw-bold';
                            span.title = `Below min by ${minVal - stock}`;
                        } else if (maxVal > 0 && stock > maxVal) {
                            span.className = 'text-warning fw-bold';
                            span.title = `Exceeds max by ${stock - maxVal}`;
                        }
                        cells[7].innerHTML = '';
                        cells[7].appendChild(span);
                    }
                    
                    // If stock exceeds max, show an alert toast
                    if (reorderMax > 0 && stock > reorderMax) {
                        showAlert(`Qty on hand exceeds maximum by ${stock - reorderMax}. Consider reducing stock or updating max.`, 'warning');
                    }
                }
...
<?php echo $__env->make('layouts.admin_app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\JJ_Flowershop_Capstone\frontend\resources\views/admin/inventory.blade.php ENDPATH**/ ?>